"""Train an AGCRN-style model from a disk-backed window store."""
...
from xinyang_wind15.agcrn import AGCRNSeq2One  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        default=str(PROJECT_DIR / "artifacts" / "local_debug" / "agcrn_store_train"),
    )
...
def main() -> None:
    ...
    if len(train_origins) == 0 or len(val_origins) == 0 or len(test_origins) == 0:
        raise ValueError(
            "AGCRN store training requires non-empty train/val/test windows. "
    ...
    if best_state is None:
        raise RuntimeError("AGCRN store training finished without a valid checkpoint.")
    ...
    save_checkpoint(model, output_dir / "agcrn_baseline.pt")
