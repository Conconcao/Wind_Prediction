"""Train a local TCN baseline on xinyang 15-minute wind-speed windows."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        default=str(PROJECT_DIR / "artifacts" / "local_debug" / "tcn_smoke"),
    )
...
def main() -> None:
    ...
    if not train_mask.any() or not val_mask.any() or not test_mask.any():
        raise ValueError(
            "TCN training requires non-empty train/val/test windows. "
    ...
    if best_state is None:
        raise RuntimeError("TCN training finished without a valid checkpoint.")
